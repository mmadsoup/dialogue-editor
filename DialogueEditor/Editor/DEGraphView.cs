...
public class DEGraphView : GraphView
{
    ...
    public List<DEYarnVariable> YarnVariables;
    ...
    private void AddBlackboard()
    {
        ...
        {
            title = "Yarn Variables",
            subTitle = string.Empty,
            scrollable = true
        };
        blackboard.style.backgroundColor = new Color(34f / 255f, 34f / 255f, 34f / 255f);
        blackboard.SetPosition(new Rect(15, 300, 200, 300));

        blackboard.addItemRequested = _blackboard =>
        {
            AddPropertyToBlackboard(new DEYarnVariable(), blackboard);
        };

        Add(blackboard);
    }

    private void AddPropertyToBlackboard(DEYarnVariable yarnVariable, Blackboard blackboard)
    {
        DEYarnVariable property = new()
        {
            VariableName = yarnVariable.VariableName,
            VariableValue = yarnVariable.VariableValue
        };
        YarnVariables.Add(property);

        VisualElement container = new();
        BlackboardField blackboardField = new()
        { 
            text = property.VariableName
        };
        container.Add(blackboardField);

        TextField valueTextField = new()
        { 
            value = property.VariableValue
        };

        valueTextField.RegisterValueChangedCallback(value =>
        {
